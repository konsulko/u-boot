...
static const struct stm32_gpio_dsc usart2_gpio[] = {
	{STM32_GPIO_PORT_A, STM32_GPIO_PIN_2},	/* TX */
	{STM32_GPIO_PORT_A, STM32_GPIO_PIN_3},	/* RX */
};
...
int uart2_setup_gpio(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(usart2_gpio); i++) {
		rv = stm32_gpio_config(&usart2_gpio[i], &gpio_ctl_usart[i]);
		if (rv)
			goto out;
	}
